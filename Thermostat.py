    ...
    def updateLights(self):
        ...
        if self.current_state.id == "heat":
            if temp < self.setPoint:
                # Active heating: red LED fades.
                redLight.pulse(fade_in_time=1, fade_out_time=1)
            else:
                # Temperature met/exceeded: red LED solid.
                redLight.on()
        elif self.current_state.id == "cool":
            if temp > self.setPoint:
                # Active cooling: blue LED fades.
                blueLight.pulse(fade_in_time=1, fade_out_time=1)
            else:
                # Temperature met or below: blue LED solid.
                blueLight.on()
        elif self.current_state.id == "off":
            # Off state: both LEDs remain off.
            redLight.off()
            blueLight.off()
    ...
    def manageMyDisplay(self):
        ...
        while not self.endDisplay:
            ...
            if(altCounter < 6):
                ...
                lcd_line_2 = f"{self.current_state.id[:4]}:{self.setPoint}F"
                ## TODO: Add the code necessary to setup the second line
            ...
            screen.updateScreen(lcd_line_2)
